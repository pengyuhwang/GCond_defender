...
class GCond:
    ...
    def __init__(self, data, args, device='cuda', **kwargs):
        ...
        # 计算合成图节点数：原始训练节点数 × reduction_rate
        n = int(len(data.idx_train) * args.reduction_rate)
        d = data.feat_train.shape[1]

        self.nnodes_syn = n
        # 合成特征：可学习的参数（nn.Parameter）
        self.feat_syn = nn.Parameter(torch.FloatTensor(n, d).to(device))
        # PGE：用神经网络从 feat_syn 生成 adj_syn
        ...
        # 两个优化器：分别优化 feat_syn 和 PGE（即 adj_syn）
        ...
        for it in range(args.epochs + 1):
            ...
            for ol in range(outer_loop):
                ...
                for c in range(data.nclass):
                    ...
                    # 原始图采样子图上前向传播，计算损失 loss_real。
                    # 计算真实梯度 gw_real（对 GNN 参数的梯度），并 detach（不参与后续反传）
                    ...
                    # 在整个合成图上前向传播。
                    # 切片出类别 c 对应的输出，计算 loss_syn。
                    # 计算合成梯度 gw_syn，注意 create_graph=True（因为后续要对 feat_syn/pge 求导）
                    ...
                    # match_loss：通常是 MSE 或 cosine 距离，衡量两组梯度的相似度。
                    # coeff：按类别样本数加权（避免大类主导）。
                ...
                # 交替优化：每 50 轮中，前 10 轮更新 pge（adj），后 40 轮更新 feat
                if it % 50 < 10:
                ...
                # 在合成图上实际训练几步 GNN（模拟真实训练过程）。
                # 这是为了让 GNN 参数更“适应”当前合成图，使梯度匹配更准确。
                # 注意：这里更新的是 model 的参数，不影响合成图。
